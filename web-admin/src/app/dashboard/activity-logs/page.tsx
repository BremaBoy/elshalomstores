...
export default function ActivityLogsPage() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const { data: adminLogs } = await supabaseAuth.from('admins').select('id, name, created_at').order('created_at', { ascending: false }).limit(10)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Real-time audit trail of all platform administrative actions</p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-2xl">
         <div className="p-6 border-b border-border flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Live Stream Active</span>
            </div>
         </div>
         ...
         <div className="divide-y divide-border">
            {isLoading ? (
               ...
               <div key={log.id} className="p-5 flex items-start gap-5 hover:bg-accent transition-colors group">
                  <div className={`p-3 rounded-2xl flex-shrink-0 ${log.color} border border-white/5`}>
                     <log.icon className="w-5 h-5" />
                  </div>
                  <div className="flex-1">
                     <div className="flex items-center justify-between gap-4">
                        <h4 className="font-bold text-foreground text-sm">{log.action}</h4>
                        <span className="text-[10px] text-muted-foreground font-medium flex items-center gap-1.5 whitespace-nowrap">
                     ...
                     <div className="mt-3 flex items-center gap-2">
                        <div className="w-5 h-5 rounded-full bg-accent border border-border flex items-center justify-center text-[10px] font-bold text-muted-foreground">
                           S
                        </div>
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">System Process</span>
                     </div>
                  </div>
