    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage store revenue and payment processing</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-6 relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold text-foreground mb-2">₦{pendingRevenue.toLocaleString()}</p>
        ...
        <div className="bg-card border border-border rounded-xl p-6 relative overflow-hidden group">
          ...
          <p className="text-3xl font-bold text-foreground mb-2">Paystack / FW</p>
          <p className="text-xs text-muted-foreground font-medium">Active Gateways</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr key={o.id} className="hover:bg-accent transition-colors group">
                  <td className="px-6 py-4">
                    <p className="font-mono text-[10px] text-primary uppercase font-bold truncate max-w-[120px]" title={o.id}>#{o.id.split('-')[0]}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{o.payment_method || 'Online Payment'}</p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="font-medium text-foreground text-xs">{o.users?.name || 'Guest'}</p>
                    <p className="text-[10px] text-muted-foreground truncate max-w-[150px]">{o.users?.email}</p>
                  </td>
                  <td className="px-6 py-4">
                    <p className="font-bold text-foreground">₦{o.total_amount?.toLocaleString()}</p>
                  </td>
                  <td className="px-6 py-4">
