...
export const WishlistClient = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-8">
        ...
        <div className="text-center space-y-3 max-w-md">
          <h4 className="text-2xl font-extrabold uppercase tracking-tight text-text-primary">
            Nothing saved yet
          </h4>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {items.map((item) => (
        <div key={item.id} className="group relative bg-card border border-border rounded-3xl p-4 flex flex-col gap-4 transition-all hover:shadow-xl hover:shadow-black/10">
          <Link href={`/product/${item.id}`} className="block relative aspect-square rounded-2xl overflow-hidden bg-bg">
            <Image
              src={item.image}
              alt={item.name}
              fill
          ...
          <div className="flex-1 space-y-2">
            ...
            <Link href={`/product/${item.id}`} className="block">
              <h4 className="font-bold text-text-primary line-clamp-1 group-hover:text-primary transition-colors">{item.name}</h4>
            </Link>
