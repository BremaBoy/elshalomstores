              ...
              return (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-text-secondary dark:text-white/45">{copy as string}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="grid gap-4 lg:grid-cols-[230px_minmax(0,1fr)_230px]">
              ...
              <nav className="space-y-0.5">
                {categories.slice(0, 8).map((category, index) => (
                  <Link
                    ...
                    className="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-bold text-text-secondary transition hover:bg-primary/[.08] hover:text-text-primary dark:text-white/75 dark:hover:bg-white/10 dark:hover:text-white"
                  >
                    <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/10 text-sm dark:bg-white/10">
                      {category.icon || categoryIcons[index]}
                    </span>
                    <span className="min-w-0 flex-1 truncate">{category.name}</span>
                    <ChevronRight className="h-3.5 w-3.5 text-primary/30 transition group-hover:translate-x-0.5 group-hover:text-primary dark:text-white/20" />
                  </Link>
                ))}
              </nav>
              <Link
                href="/categories"
                className="mt-2 flex items-center gap-2 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-primary dark:text-violet-300"
              >
            ...
            <div className="group relative min-h-[460px] overflow-hidden rounded-[2rem] border border-border bg-[#EAE1D5] shadow-sm transition-colors dark:border-white/10 dark:bg-zinc-900 dark:shadow-none">
              ...
              <div className="absolute inset-0 flex max-w-2xl flex-col justify-center p-7 md:p-12">
                ...
                <p className="mt-5 max-w-lg text-sm leading-6 text-text-secondary dark:text-white/65 md:text-base">
                  {hero.subtitle}
                </p>
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <Link
                    href="/categories"
                    className="inline-flex h-12 items-center rounded-xl border border-text-primary/20 bg-white/35 px-6 text-xs font-black uppercase tracking-[.14em] text-text-primary backdrop-blur transition hover:bg-white/60 dark:border-white/20 dark:bg-white/5 dark:text-white dark:hover:bg-white/10"
                  >
                    Browse categories
                  </Link>
                </div>
            ...
            <aside className="grid grid-cols-2 gap-3 lg:grid-cols-1">
              ...
              >
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-text-secondary dark:text-white/45">Talk to our shopping support team.</p>
                </div>
              </Link>
            </aside>
          </div>
